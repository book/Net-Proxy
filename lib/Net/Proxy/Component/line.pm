...
sub m_DATA {
    ...
    while ( $data =~ s/(.*?(?:\015\012?|\012\015?))// ) {
        push @messages, Net::Proxy::Message->new( m_DATA => { data => $1 } );
    }
...
A C<m_DATA> message is split in several C<m_DATA> messages (ending with
either C<CR>, C<LF>, C<CR-LF> or C<LF-CR>).

If it doesn't end with a newline, the last chunk of data in the message
will be kept and prepended to the content of the next C<m_DATA> message.
