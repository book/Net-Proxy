...
=head2 A basic C<Net::Proxy> script

Most C<Net::Proxy> based scripts look like the following:
...
The individual proxies are registered with the C<Net::Proxy> framework:
...
Note: The C<set_verbosity()> method is available only since C<Net::Proxy>
version 0.04.
...
=head2 The concepts behind C<Net::Proxy>
...
C<Net::Proxy> introduces the concept of "connectors". Connectors are
...
and C<[in]> (left) and C<[out]> (right) the C<Net::Proxy> connectors.
...
The simplest connector is named C<Net::Proxy::Connector::tcp> (we'll
use C<tcp> for short). When placed in the C<in> side, it simply
C<listen()>s for incoming connections and them C<accept()>s them.
Then the C<out> connector C<connect()>s to the server.

Each connector accepts different parameters
...
(This example requires at least C<Net::Proxy> version 0.02 to work.)
...
Here's how to set up the local C<Net::Proxy> instance:
...
to get through web proxies, so C<Net::Proxy> probably isn't necessary
any longer to handle this kind of traffic.


=head2 Running two services on the same TCP port

(This example requires at least C<Net::Proxy> version 0.03 to work.)
...
C<Net::Proxy>'s C<dual> connector is able to detect between two such clients
...
(This example requires at least C<Net::Proxy> version 0.06 to work.)
...
the following signature:
    ...
    # Net::Proxy version 0.06
    sub {
        my ( $dataref, $connector ) = @_;
        ...
    }

    # As from Net::Proxy version 0.07
    sub {
        my ( $dataref, $connector, $socket ) = @_;
        ...
    }

The ROT-13 routine is straightforward:
...
(This example requires at least C<Net::Proxy> version 0.08 to work.)
